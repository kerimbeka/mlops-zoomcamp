...
def calculate_metrics_postgresql(i):
    ...
    quantile_value = result["metrics"][3]["value"]
    min_value = result["metrics"][4]["value"]
    with psycopg.connect(CONNECTION_STRING_DB, autocommit=True) as conn:
        with conn.cursor() as curr:
            curr.execute(
                "insert into dummy_metrics(timestamp, prediction_drift, num_drifted_columns, share_missing_values, quantile_value, min_value) values (%s, %s, %s, %s ,%s, %s)",
                (
